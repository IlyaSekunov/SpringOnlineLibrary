...
public class BookDAO {
    ...
    public void addBook(Book book) {
        jdbcTemplate.update("insert into book(title, author, publish_year) values(?, ?, ?)",
    ...
    public Person getBookOwnerByBookId(int bookId) {
        return jdbcTemplate.query("select person.* from person join book on book.person_id = person.person_id where book.book_id = ?",
    ...
    public void updateBook(int id, Book book) {
        jdbcTemplate.update("update book set title = ?, author = ?, publish_year = ? where book_id = ?",
                book.getTitle(), book.getAuthor(), book.getPublishYear(), id);
    }
